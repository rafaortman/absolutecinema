...
function syncInputs(){
  ...
  $('favoritesButton').setAttribute('aria-pressed',String(state.favoritesOnly)); $('favoritesButton').innerHTML=heartSvg(state.favoritesOnly);
}
...
const HEART_FILL_SVG='<svg viewBox="0 -960 960 960" aria-hidden="true"><path d="m480-120-58-52q-101-91-167-157T150-447.5Q111-500 95.5-544T80-634q0-94 63-157t157-63q52 0 99 22t81 62q34-40 81-62t99-22q94 0 157 63t63 157q0 46-15.5 90T810-447.5Q771-395 705-329T538-172l-58 52Z"/></svg>';
const heartSvg=on=>on?HEART_FILL_SVG:HEART_SVG;
...
function syncFav(){if(!shownMovie)return;const on=favorites.has(shownMovie.id);$('favMovie').classList.toggle('active',on);$('favMovie').innerHTML=heartSvg(on);}
...
async function boot(){
  ...
  const withStream=movies.filter(m=>m.streaming?.subscription?.length).length;
  const pct=Math.round(withStream/movies.length*100);
  if(checked.length){const[y,mo,d]=checked[checked.length-1].split('-');$('streamingUpdated').textContent=`${withStream} de ${movies.length} filmes disponíveis (${pct}%), atualizados em ${d}/${mo}/${y}`;}
