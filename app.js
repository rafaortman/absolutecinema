...
function setView(view, reset=true){
  ...
  $('viewDescription').textContent=view==='lists'?'Grandes listas reunidas em uma só curadoria. Veja onde elas concordam ou siga a sua favorita.':'Dos clássicos às descobertas recentes, percorra os filmes reconhecidos por Cannes e pelo Oscar.';
...
function draw(){const pool=drawPool();$('drawResult').innerHTML=pool.length?card(pool[Math.floor(Math.random()*pool.length)]):'<div class="empty">Nenhum filme com esses critérios.</div>';}
